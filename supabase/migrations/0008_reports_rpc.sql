-- ── Migration 0008: get_reports_data RPC ─────────────────────────────────
--
-- Returns one row per active SKU with:
--   • sales totals (revenue, qty, avg price) for the given date range
--   • current landed cost (from most recent in-stock batch)
--   • gross margin % = (avg_sell - landed) / avg_sell × 100
--   • current stock (pieces) from stock_movements ledger
--   • days_of_stock = stock / daily_avg_sold (null if no sales in period)
--
-- All SKUs are returned — those with no sales show zeros so you can see
...
AS $$
  ...
  -- Latest landed cost per SKU (most recent batch with remaining stock)
  latest_landed AS (
