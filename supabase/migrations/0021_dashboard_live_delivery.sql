-- Dashboard v3: add orders_out_for_delivery (live on-road count)
-- Replaces get_dashboard_metrics() to add the new column.
...
RETURNS TABLE (
  ...
  -- Orders
  orders_awaiting_dispatch    BIGINT,  -- status = 'confirmed'
  orders_out_for_delivery     BIGINT,  -- status = 'out_for_delivery' right now (live)
  orders_dispatched_today     BIGINT,  -- status = 'out_for_delivery', updated today
  orders_delivered_today      BIGINT,  -- status = 'delivered', delivered today
  overdue_orders_count        BIGINT,  -- confirmed > 24 h without dispatch
  -- Stock
...
AS $$
  ...
  -- Live: currently out for delivery (on the road right now)
  ...
  -- Dispatched today (became out_for_delivery today)
  dispatched_today AS (
    SELECT COUNT(*) AS cnt
    FROM sales_orders
    WHERE status = 'out_for_delivery'
  ...
  -- Awaiting dispatch (confirmed but not yet dispatched)
  ...
  -- Overdue: confirmed for more than 24 hours
  overdue AS (
    SELECT COUNT(*) AS cnt
    FROM sales_orders
    WHERE status = 'confirmed'
  ...
  SELECT
    ...
    -- Orders currently on the road (live)
