-- Dashboard v6: add shipments_arriving_soon
-- Shipments with status = 'in_transit' and expected_arrival_date within 3 days.
-- This gives the owner advance warning to prepare the godown.
...
RETURNS TABLE (
  ...
  shipments_arriving_soon     BIGINT   -- in_transit, expected_arrival within 3 days
)
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  ...
  -- Shipments arriving within 3 days — owner needs to prepare godown
  arriving_soon AS (
  ...
  stock_val AS (
    ...
    LEFT JOIN (
      SELECT batch_id, SUM(qty_pieces) AS out_pieces FROM stock_movements
      WHERE movement_type IN ('out','damage_out','transfer_out') GROUP BY batch_id
