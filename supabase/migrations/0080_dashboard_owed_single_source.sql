-- ============================================================================
-- 0080 — Dashboard "owed" = the exact same number as the Finance Owed panel
-- ============================================================================
-- Ali's screenshot: Finance › Owed showed 2 customers owing MVR 1,490, but the
-- dashboard showed only 1. Both were internally correct but answered DIFFERENT
-- questions under the same word "unpaid/owed":
--
--   • Finance Owed (get_receivables_aging): the true receivable — every
--     non-draft/non-cancelled unpaid order, balance = order total MINUS
--     payments already made. Recognises the debt at confirmation, which is
--     also when get_pnl recognises the revenue. This is the correct figure.
--
--   • Dashboard pending_pay: only status='delivered' orders, and it summed the
--     FULL order total ignoring partial payments. So it (a) missed a
--     confirmed-but-not-yet-delivered unpaid order — the 2nd customer — and
--     (b) would overstate any part-paid order.
--
-- A money app must have ONE definition of "owed". This repoints the dashboard
-- at get_receivables_aging() so its figure is, by construction, the sum of the
-- Owed panel — they can never diverge again. Only the pending_pay CTE changes;
-- every other column is carried verbatim.
-- ============================================================================

CREATE OR REPLACE FUNCTION public.get_dashboard_metrics()
 RETURNS TABLE(revenue_today_mvr numeric, revenue_this_month_mvr numeric, revenue_last_month_mvr numeric, gross_profit_this_month_mvr numeric, gross_margin_pct numeric, orders_awaiting_dispatch bigint, orders_out_for_delivery bigint, orders_dispatched_today bigint, orders_delivered_today bigint, overdue_orders_count bigint, low_stock_sku_count bigint, total_stock_value_mvr numeric, shipments_in_transit bigint, pending_payments_mvr numeric, pending_payments_count bigint, cod_undeposited_mvr numeric, shipments_arriving_soon bigint, overstock_sku_count bigint, reorder_needed_count bigint)
 LANGUAGE sql
...
AS $function$
  ...
  -- 0080: "owed" now means the SAME thing as the Finance Owed panel — the full
  -- receivable (order total minus payments) on every non-draft/cancelled unpaid
  -- order, sourced from the one canonical function so the two can't disagree.
  pending_pay AS (
